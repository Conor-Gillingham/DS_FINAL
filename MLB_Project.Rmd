...
# Convert integer columns to numeric if necessary
MLB[] <- lapply(MLB, function(x) if (is.integer(x)) as.numeric(x) else x)
model <- lm(X2023_Total_Payroll ~ `Win_Percent` + `Overall_Average` + Rank, data = MLB)
...
model_log_transformed <- lm(Win_Percent ~ log_X2023_Total_Payroll + log_Overall_Average + Rank, data = MLB)
